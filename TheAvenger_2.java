 ...
 */
public class TheAvenger_2 implements ActionController {
    ...
    public CardData openingBuyCards(List<CardData> buyCardOptions) {
        if (getCard(buyCardOptions, CardName.MINE).isPresent()) { // 5 cost
            ...
            if (KingdomAPI.getCardCountRemaining(CardName.MILITIA) != 10) {
                return getCard(buyCardOptions, CardName.MOAT).get();
            } else {
                return getCard(buyCardOptions, CardName.CELLAR).get();
            }
        }
    }
    ...
    public CardData buyCardHook(List<CardData> buyOptions) {
        ...
        boolean delayProvince = KingdomAPI.getCardCountRemaining(CardName.PROVINCE) == 12 && expectedGold() < 8;

        if (getCard(buyOptions, CardName.PROVINCE).isPresent() && !delayProvince) {
        ...
        if (getCard(buyOptions, CardName.SILVER).isPresent()
                && expectedGold < 8
                && getCard(buyOptions, CardName.REMODEL).isEmpty()) {
        ...
        boolean buyNonTerminals = terminalCount > 4 * nonTerminals;

        if (expectedActions() > expectedActionCards() + 1) {
            if (buyNonTerminals) {
                distribution.put(CardName.VILLAGE, 30);
            } else {
                distribution.put(CardName.MARKET, Math.min(100, RoundAPI.getRoundNumber() * 10));
                distribution.put(CardName.MERCHANT, 100 - (RoundAPI.getRoundNumber() * 20));
                distribution.put(CardName.MILITIA, 200 - RoundAPI.getRoundNumber() * 50);
                ...
                distribution.put(CardName.MOAT, 200 - RoundAPI.getRoundNumber() * 50);
